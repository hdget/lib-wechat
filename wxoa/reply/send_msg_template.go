...
func NewTemplateSendMessage(redisProvider intf.RedisProvider, arg *SendMessageTemplateArgument) (SendMessager, error) {
...
func (m templateSendMessageImpl) Send(contents map[string]string) error {
	accessToken, err := wxoa.New(m.redis, m.arg.AppId, m.arg.AppSecret).GetAccessToken()
	if err != nil {
		return err
	}
