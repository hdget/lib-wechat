...
// nolint:recheck
func (impl *wxoaImpl) CreateJsSdkSignature(url string) (*JsSdkSignatureResult, error) {
...
func (impl *wxoaImpl) getJsSdkTicket() (string, error) {
	...
	if ticket == "" {
		...
		ticketResult, err := impl.api.GetJsSdkTicket(accessToken)
		if err != nil {
			return "", err
		}
...
// 生成微信签名
func (impl *wxoaImpl) createJsSdkSignature(ticket, url string) (*JsSdkSignatureResult, error) {
	now := time.Now().Unix()
