...
func (impl serviceProviderWxApiImpl) SetAuthorizerOption(authorizerAccessToken string, optionName string, optionValue string) error {
	...
	ret, err := wxapi.Post[wxapi.Result](url, req)
	if err != nil {
		return errors.Wrap(err, "get authorizer option")
	}

	if err = wxapi.CheckResult(ret, url, req); err != nil {
		return errors.Wrap(err, "get authorizer option")
	}
